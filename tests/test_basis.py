...
class TestSharpFourierZernikeBasis:
    ...
    def test_evaluate_derivatives_number0_matches_standard_basis(self, derivatives):
        """Test SharpFourierZernikeBasis derivatives for number=0 against standard basis."""
        nodes = LinearGrid(L=2, M=2, N=2).nodes
        basis_sharp = SharpFourierZernikeBasis(
            L=2, M=2, N=2, NFP=1, m_b=4, n_b=4, number=0
        )
        basis_std = FourierZernikeBasis(L=2, M=2, N=2, NFP=1)

        output_sharp = np.asarray(
            basis_sharp.evaluate(nodes, derivatives=np.array(derivatives))
        )
        output_std = np.asarray(
            basis_std.evaluate(nodes, derivatives=np.array(derivatives))
        )

        np.testing.assert_allclose(output_sharp, output_std, atol=1e-11)

    @pytest.mark.unit
    def test_evaluate_derivatives_finite_difference_sharp_mapping(self):
        """Test SharpFourierZernikeBasis derivative output using finite differences."""
        basis = SharpFourierZernikeBasis(
            L=2,
            M=1,
            N=1,
        ...
        nodes = np.array([[0.6, 0.9, 0.4], [0.35, 2.0, 1.7]])

        def central_difference(nodes, axis):
            h = 1e-6
            nodes_p = nodes.copy()
            nodes_m = nodes.copy()
            nodes_p[:, axis] += h
            nodes_m[:, axis] -= h
            f_p = np.asarray(basis.evaluate(nodes_p))
            f_m = np.asarray(basis.evaluate(nodes_m))
            return (f_p - f_m) / (2 * h)

        analytic_dr = np.asarray(basis.evaluate(nodes, derivatives=[1, 0, 0]))
        analytic_dt = np.asarray(basis.evaluate(nodes, derivatives=[0, 1, 0]))
        analytic_dz = np.asarray(basis.evaluate(nodes, derivatives=[0, 0, 1]))

        approx_dr = central_difference(nodes, 0)
        approx_dt = central_difference(nodes, 1)
        approx_dz = central_difference(nodes, 2)

        np.testing.assert_allclose(analytic_dr, approx_dr, atol=1e-6, rtol=1e-5)
        np.testing.assert_allclose(analytic_dt, approx_dt, atol=1e-6, rtol=1e-5)
        np.testing.assert_allclose(analytic_dz, approx_dz, atol=1e-6, rtol=1e-5)
