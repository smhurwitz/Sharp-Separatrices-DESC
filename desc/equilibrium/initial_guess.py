...
from desc.geometry import FourierRZCurve, Surface, VolumeRegion
...
def set_initial_guess_sharp(eq, *args, ensure_nested=True):  # noqa: C901
    ...
    args :
        either:
          - No arguments, in which case the initial guess will be set to eq.volume and 
            eq.L_lmn will be set to zero.
          - Another VolumeRegion object
          - Another SharpEquilibrium, whose flux surfaces and lambda will be used.
          - Grid and 2-3 ndarrays, specifying the flux surface locations (R, Z, and
            optionally lambda) at fixed flux coordinates. All arrays should have the
            same length. Optionally, an ndarray of shape(k,3) may be passed instead
            of a grid. If lambda is not passed, it will be set to zero.
    ...
    if nargs > 3:
        raise ValueError(
            "set_initial_guess should be called with 3 or fewer arguments."
        )
    if nargs == 0 or nargs == 1 and args[0] is None:
        if hasattr(eq, "_volume"):
            eq.R_lmn = eq.volume.R_lmn
            eq.Z_lmn = eq.volume.Z_lmn
            eq.L_lmn = np.zeros_like(eq.L_lmn)
        else:
            raise ValueError(
                "set_initial_guess called with no arguments, "
                + "but no volume is assigned."
            )
    else:  # nargs > 0
        if isinstance(args[0], VolumeRegion):
            volume = args[0]
            eq.R_lmn = volume.R_lmn
            eq.Z_lmn = volume.Z_lmn
            eq.L_lmn = np.zeros_like(eq.L_lmn)
        elif type(args[0]) is type(eq):
            eq1 = args[0]
            if nargs > 1:
